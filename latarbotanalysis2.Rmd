...
ggplot(displaydata1_noload_exemplar, aes(x = index_fixture, y = display_latency_us_otlrsrmvd)) +
  ...
  geom_line() +
  geom_point() + 
  annotate("text", x = 85, y = 35000, size = 5.5, label = "Actual Difference = 31,777 μs") + 
  annotate("text", x = 85, y = 30000, size = 5.5, label = "Theoretical Difference = 16,666 μs") + 
  theme_classic() +
  theme(text = element_text(size = 22), 
        strip.background = element_blank()) + coord_cartesian(xlim = c(0, 100), ylim = c(0, 80000))

# exemplar (Samsung SM-G960U/iPhone12_1 (120Hz example))
capacitivedata1_noload_exemplar <- subset(capacitivedata1_noload, phone_model == "Samsung SM-N970U1" & procedure == "199")
...
ggplot(capacitivedata1_noload_exemplar, aes(x = index_fixture, y = action_latency_us_otlrsrmvd)) +
  ...
  geom_line() +
  geom_point() +
  annotate("text", x = 85, y = 18500, size = 5.5, label = "Actual Difference = 9,118 μs") + 
  annotate("text", x = 85, y = 17500, size = 5.5, label = "Theoretical Difference = 8,333 μs") + 
  theme_classic() +
  theme(text = element_text(size = 22), 
        strip.background = element_blank()) + coord_cartesian(xlim = c(0, 100))
#, ylim = c(0, 20000)
```
...
display_bounds <- displaydata1_noload_199_box %>%
  ...
  subset(., latencyType == "callback", select = c("Model", "minDisplayBound", "maxDisplayBound"))
...
displaydatafull_agg %>% subset(., select = c(display_latency_us_otlrsrmvd, callback_latency_us_otlrsrmvd, callbackActionDiff, Geekbench_5_single_core, Display_refresh_Hz, phone_age, MSRP_USD)) %>%
  setnames(., old = c("display_latency_us_otlrsrmvd", "callback_latency_us_otlrsrmvd", "callbackActionDiff", "Geekbench_5_single_core", "Display_refresh_Hz", "phone_age", "MSRP_USD"), new = c("Display Latency (μs)", "Callback Latency (μs)", "Callback - Display (μs)", "Geekbench 5 Score", "Display Refresh (Hz)", "Phone Age (Yrs)", "Cost (MRSP; USD)")) %>%
pairs.panels(stars = T, lm = T, ci = T, method = "pearson", hist.col = "#00AFBB", density = T, ellipses = F, cex.cor = 10, main = "(A) Device Characteristics and Display Latency Correlations", cex = 2, cex.labels = 1.2, cex.main = 2)
...
capacitivedatafull_agg %>% subset(., select = c(action_latency_us_otlrsrmvd, callback_latency_us_otlrsrmvd, callbackActionDiff, Geekbench_5_single_core, Touch_refresh_Hz, phone_age, MSRP_USD)) %>%
  setnames(., old = c("action_latency_us_otlrsrmvd", "callback_latency_us_otlrsrmvd", "callbackActionDiff", "Geekbench_5_single_core", "Touch_refresh_Hz", "phone_age", "MSRP_USD"), new = c("Action Latency (μs)", "Callback Latency (μs)", "Callback - Action (μs)", "Geekbench 5 Score", "Touch Sampling (Hz)", "Phone Age (Yrs)", "Cost (MRSP; USD)")) %>%
pairs.panels(stars = T, lm = T, ci = T, method = "pearson", hist.col = "#00AFBB", density = T, ellipses = F, cex.cor = 10, main = "(B) Device Characteristics and Capacitive Tap Latency Correlations", cex = 2, cex.labels = 1.2, cex.main = 2)
```
...
T2data %>% subset(., select = c("OS", "action_latency_us_otlrsrmvd", "tapcallback_latency_us_otlrsrmvd", "tapcallbackActionDiff", "display_latency_us_otlrsrmvd", "dispcallback_latency_us_otlrsrmvd", "dispcallbackActionDiff", "Geekbench_5_single_core", "Touch_refresh_Hz", "Display_refresh_Hz", "phone_age", "MSRP_USD")) %>%
  ...
  modify_header(label = "** **") %>%
  modify_caption("**Table 1. OS Differences**") %>%
  bold_labels()
```
